...
const replaceMatchedOrg = async (match: string, group1: string, group2: string): Promise<string> => generatedMarkdown.generateReportingStructure(group2)

const replaceAsync = async (markdown: string, regex: RegExp, replacer: (string: string, ...args: string[]) => Promise<string>): Promise<string> => {
...
async function insertGeneratedMarkdown(markdown: string): Promise<string> {
    if (markdown.match(/{{generator:/)) {
        ...
        markdown = await replaceAsync(markdown,
            /({{generator:reporting_structure.)(\w+)(}})/gi,
            replaceMatchedOrg
        )
        markdown = markdown.replace(
