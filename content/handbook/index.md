...
- **The Handbook is a source of truth** at Sourcegraph, and information there is expected to be accurate and up-to-date. This includes for different processes the company uses, for something to be in force as a practice here at Sourcegraph, it should be in the handbook. A Google Doc or pending PR to the handbook is not sufficient as a source of truth because using those reduces the effectiveness of the handbook as a trusted source of knowledge. Further, by never needing to merge your change to bring it into force, it discourages making small, iterative improvements.
- **Use PRs to discuss changes to the handbook.** Whenever possible (an exception for example might be when discussing non-public information that might lead to a handbook update), use PRs to the handbook to propose changes to the handbook. When using a Google Doc instead it can be easy to miss that there's existing (even conflicting) content, and by getting to a PR more quickly, it's easier to discover these and for others to contribute cross-links or highlight duplicated/conflicting content.
