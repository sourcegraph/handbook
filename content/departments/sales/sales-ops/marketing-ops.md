...
5. Values for these fields will be the same for each campaign member manually added and will vary by campaign type. Here are examples for a sponsored live event:
    ...
    10. Lead Source: Event
    11. Recent Person Source Category: Inbound
    12. Recent Person Source: Live Event 
    13. Recent Person Source Details: Sponsored
    14. Person Source Category: Inbound
    15. Person Source: Live Event
    16. Person Source Details: Sponsored
...
9. The next screen will show HubSpot’s attempts at automatically mapping the uploaded columns to pre-existing contact properties. Some of these will need to be manually changed as follows:
    ...
    18. Recent Person Source maps to “Person Source”
    19. Recent Person Source Detail maps to “Person Source Detail”
    20. Person Source Category maps to “Original Person Source Category”
    21. Person Source maps to “Original Person Source”
    22. Person Source Detail maps to “Original Person Source Detail”
...
Our GTM analytics are driven by the following BigQuery scheduled queries:
* [Sfdc_lead_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/631c6d54-0000-2efb-83fc-f40304363ab6/runs?project=telligentsourcegraph)
    * Purpose: Combine out-of-the-box SFDC lead object fields with supplemental transformations used by Sales and Finance for summarizing prospecting performance. 
    * Core transformations: Categorizing various title keywords into 4 key buckets utilized by Sales for prospecting; creating flags for recent product sign-ups, usage, etc.  
    * Core joins: Joined to user table to match employee IDs to full names/emails (three separate joins to pull AE, SDR, and owner names); joined to account table to convert account IDs to legal names.  
* [Sfdc_contact_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/63c047a4-0000-2e04-8e9f-14223bafe756/runs?project=telligentsourcegraph) 
    * Purpose: Same as above but for contact object in SFDC. Note that most prospects come into our system as leads and are later converted to contacts by a member of the Sales team. As a result, the lead object contains the majority of information we care about.
    * Core transformations: N/A
    * Core joins: Same as above.
* [Sfdc_campaign_member_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/631d584a-0000-2000-910b-089e0831d5b0/runs?project=telligentsourcegraph) 
    * Purpose: Same as above but for campaign member object in SFDC. Campaign memberships are important because one lead/contact can be a member of multiple campaigns - looking at the lead object alone will not illustrate the level of interactions actually being taken with our marketing materials, products, etc. 
    * Core transformations: Categorizing various title keywords into 4 key buckets utilized by Sales for prospecting; overall campaign response date corrects for manual data fixes that will show the date of the fix vs. the actual date of campaign membership in the first_responded_date field; creating flags for recent responses to campaigns to see which prospects should be prioritized at-a-glance.
    * Core joins: Joined to campaign table to convert campaign ID to campaign name; joined to account table to convert account IDs to legal names.
* [Sfdc_account_custom_datat_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/63261af6-0000-2adf-94d4-883d24f7d888/runs?project=telligentsourcegraph) 
    * Purpose: To make summary account information clear.
    * Core transformations: Flags for accounts that are currently assigned to an active AE.
    * Core joins: Joined to user table 3 times to translate IDs to names for assigned AEs, SDRs, CEs, and account owners; joined to clearbit table to highlight clearbit enrichment metrics on employees, engineers, etc. on the account; joined to user role id table to bring in info on titles for filtering in Looker by employee role.
* [Sfdc_opportunity_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/6321efb0-0000-2c33-b02e-582429af17ac/runs?project=telligentsourcegraph) 
    * Purpose: Combine out-of-the-box SFDC opportunity object fields with supplemental transformations used by Sales and Finance for summarizing pipeline generation progress vs. plan. 
    * Core transformations: New fields for common queries like days from creation to close, days from qualification to close, etc. to be used in summary average calculations in Looker; flags for opportunities passing stages. 
    * Core joins: Same as above.
* [Sfdc_task_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/63f9f9b6-0000-246f-9ed4-089e082d98fc/runs?project=telligentsourcegraph) 
    * Purpose: Summarize SDR/AE call and email (tasks = calls and emails) activity to evaluate key inputs to top-of-funnel.
    * Core transformations: Categorizing various title keywords into 4 key buckets utilized by Sales for prospecting; pull in prospect info from lead/contact record; flag when activity led to new business meeting.
    * Core joins: Joined to account table to translate account ID to legal account name; joined to user and user role tables to translate employee IDs to their full names, emails, and titles for analysis in Looker; joined to lead and contact tables to translate prospect IDs to their actual identifying information for sales to leverage for prospecting. 
* [Sfdc_event_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/6393512f-0000-2272-814e-582429a7cd64/runs?project=telligentsourcegraph) 
    * Purpose: Summarize SDR/AE meeting (events = meetings) activity to evaluate key inputs to top-of-funnel.
    * Core transformations: Same as above.
    * Core joins: Same as above.
* [Outreach_mailing_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/639f390c-0000-26fa-8700-94eb2c0a6c4e/runs?project=telligentsourcegraph)  
    * Purpose: Pull info on specific email subject lines and messaging from Outreach to determine which themes are most relevant to prospects.
    * Core transformations: Flag email opens in binary form for Looker calculations.
    * Core joins: Joined to Outreach prospect email table to translate recipient ID into actual email, name, etc.; joined to Outreach sequence table to connect individual messages/emails to the specific email sequence they were sent as part of; joined to Outreach sequence step table to identify which specific step # of sequences received opens, replies, etc.; joined to sfdc user table to translate sender ID to the employee’s name, email, and title for ease of use / summary in Looker. 
* [Outreach_sequence_custom_data_export](https://console.cloud.google.com/bigquery/scheduled-queries/locations/us/configs/63add6e7-0000-28fb-bea8-14223bafb692/runs?project=telligentsourcegraph) 
    * Purpose: Summarize data on Outreach sequences leveraged by SDRs and AEs to determine which subjects, themes, etc. result in highest open and reply rates.
    * Core transformations: Same as above.
    * Core joins: Joined to Outreach users table to translate sender ID to actual employee name, email, etc.

Note that the above customized queries purposely do not include all available SFDC fields from each respective object for ease of use in Looker and clear labeling purposes. 

These scheduled queries pipe data from SFDC (and our product usage queries owned by the D&A team) into Looker and are primarily used in the following Looker folders and dashboards:
* [Sales](https://sourcegraph.looker.com/folders/527)
    * [Sales Prospecting - Key Inputs](https://sourcegraph.looker.com/boards/59)
    * [Cody Sequencing](https://sourcegraph.looker.com/dashboards/480?Sequence+Name=%25Cody%25%2C%25cody%25&Sequence+Last+Used+Date=1+months)
    * [AE Onboarding Benchmarking](https://sourcegraph.looker.com/dashboards/420?Opportunity+Type=&Division+Current=&Qualified+Date=365+day)
* [Marketing](https://sourcegraph.looker.com/folders/526)
    * [Campaign Performance](https://sourcegraph.looker.com/dashboards/469)
