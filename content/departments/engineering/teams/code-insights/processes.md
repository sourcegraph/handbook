...
1. During the design phase, the Designer creates and maintains the design changelog that is available as a separate Figma page and it is linked in the main design issue. Design Changelog is created to keep track of the key or pivotal design decisions for a given project as well as the reasoning behind them.
   1. Design Changelog is mostly useful for big projects. Smaller issues may not require creating the backlog.
   1. When re-opening the discussion about the past design decision, please read the design changelog first to make sure that your point wasn't addressed earlier and that new information has occured.
